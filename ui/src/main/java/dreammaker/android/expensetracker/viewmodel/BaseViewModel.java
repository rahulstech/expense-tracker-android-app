    ...
    protected void notifyOperationCallback(int action, boolean success, Object extra) {
        AppExecutor.getMainThreadExecutor().execute(()->{
            if (hasOperationCallback()) {
                ...
                switch (action) {
                    case Helper.ACTION_INSERT: getOperationCallback().onCompleteInsert(success);
                    break;
                    case Helper.ACTION_EDIT: getOperationCallback().onCompleteUpdate(success);
                    break;
                    case Helper.ACTION_DELETE: getOperationCallback().onCompleteDelete(success);
                }
            }
        });
    }
