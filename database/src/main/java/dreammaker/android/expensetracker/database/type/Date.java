...
public class Date implements Cloneable {
    ...
    public static Date valueOf(String dateString, String pattern) {
        if (TextUtils.isEmpty(pattern)) {
            throw new IllegalArgumentException("date pattern not provided");
        }
        try{
    ...
    public static Date today() {
        return new Date(Calendar.getInstance(Locale.ENGLISH));
    }
    ...
    public Date addDays(int q) {
        final Calendar copy = (Calendar) this.calendar.clone();
        copy.add(Calendar.DAY_OF_YEAR,q);
