...
@Deprecated
public class TransactionsAdapter extends BaseRecyclerViewPagedListAdapter<TransactionHistoryDisplayModel, TransactionsAdapter.TransactionsViewHolder>
    ...
    @Override
    protected long getItemId(@NonNull TransactionHistoryDisplayModel item) {
    ...
    public static class TransactionsViewHolder extends RVAViewHolder{
        ...
        public TransactionsViewHolder(View itemView) {
            ...
            amount = findViewById(R.id.line2);
        ...
        public void bind(TransactionHistoryDisplayModel t){
            if (Check.isNonNull(t)){
                setAmount(t.getTransaction().getAmount(), t.getType());
                setDateText(t.getTransaction().getDate());


                if (null != t.getPayeeAccount()) {
                    setAccountName(t.getPayeeAccount().getName());
                }
                else if (null != t.getPayerAccount()) {
                    setAccountName(t.getPayerAccount().getName());
                }
                if (null != t.getPayeePerson()) {
                    setPersonName(t.getPayeePerson().getName());
                }
                else if (null != t.getPayerPerson()) {
                    setPersonName(t.getPayerPerson().getName());
                }
                setDescriptionText(t.getTransaction().getDescription());
                setType(t.getType());
                hide(contentLoading);
        ...
        private void setAmount(BigDecimal amountValue, TransactionType type){
            if (type == TransactionType.DUE || type == TransactionType.EXPENSE) {
                amount.setText(amountValue.negate().toPlainString());
            }
            else {
                amount.setText(amountValue.toPlainString());
            }
        }
        ...
        private void setType(TransactionType type){
            ...
            else if (type == TransactionType.PAY_DUE || type == TransactionType.INCOME)
